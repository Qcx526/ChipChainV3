...
def classify_powerpc(i: ExportInstruction, r: dict[str, int]) -> tuple[dict, ...]:
    ...
    if m == "lwarx":
        if o: r.pop(o[0], None)
        return (memory_result(K.ATOMIC_LOAD, None),)
    if m == "stwcx.": return (memory_result(K.ATOMIC_STORE, None),)
    if m in {"rfi", "rfid"}:
        r.clear()
        return ({"kind": K.EXCEPTION_RETURN, "semantic_status": "supported"},)
    if m in {"blr", "bclr"}:
        r.clear()
