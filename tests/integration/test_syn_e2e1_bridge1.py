"""Opt-in four new real processes; default read-only replay, old B1/XL2 unchanged."""
...
from tests.integration.test_syn_e2e1_b1_mmio_grounding import real_records, test_real_xl2_unknown_byte_replay
...
def test_new_real_joint_attested_runs(monkeypatch):
    if os.environ.get('CHIPCHAIN_BRIDGE1_REAL')!='1':
        pytest.skip('explicit CHIPCHAIN_BRIDGE1_REAL=1 required for four new simulator processes')
    monkeypatch.undo()
    out=runner.execute(ROOT);evaluate(out)
    print('BRIDGE1_REAL_OUTPUT='+str(out))
